...
export class ApplicationCancelledComponent implements OnInit {
  ...
  ngOnInit() {
    this.service.getApplicationCancelledRequest().subscribe((data) => {
      if (data != null) {
  ...
  viewLeaveInfo(data: any) {
    const dialogRef = this.dialog.open(ViewLeaveCancelledReqComponent, {
      width: '500px',
...
export class ViewLeaveCancelledReqComponent implements OnInit {
  constructor(public dialogRef: MatDialogRef<ViewLeaveCancelledReqComponent>, @Inject(MAT_DIALOG_DATA) public data: any,
  ) {
