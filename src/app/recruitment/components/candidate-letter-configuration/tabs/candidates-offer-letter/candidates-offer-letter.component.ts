...
@Component({
  selector: 'app-candidates-offer-letter',
  templateUrl: './candidates-offer-letter.component.html',
  styleUrls: ['./candidates-offer-letter.component.scss']
})
export class CandidatesOfferLetterComponent implements OnInit {
  ...
  checkDocumentExist(data: any) {
    console.log(' Data For Check ::: ' + data);
    return this.existGeneratedLetterList.filter(function (object, index, arr) {
      console.log(index + ' ----- ' + object + ' -------- ' + arr);
      if (object.candidateEmailAddr === data.cndEmail) {
        return true;
      } else {
        return false;
      }
  ...
  getAllSelectedCandidate() {
    ...
    this.serviceApi.get('/v1/recruitment/candidates/' + candidateStatus).
      ...
      res => {
        res.forEach(element => {
          const dataO = this.checkDocumentExist(element);
          this.candidateApplicationsInfo.push({
            ...
            canGeneratedLetterList: element.candidateAssignLetterList
          });
        });
  ...
  generateOffterLetter(element: any) {
    $('.divtoggleDiv')[1].style.display = 'block';
    console.log('Generate Offer Letter');
    ...
    if (obj.length !== 0) {
      this.generateLetterForm = this.fb.group({
        ...
        candidateDesignation: ['Softeate Developer'],
  ...
  saveGeneratedOffterLetter() {
    console.log('Generated For candidate id ::: ' + this.generateLetterForm.value);
    const body = this.generateLetterForm.value;
    const letterType = 'OFFER_LETTER';
    console.log(JSON.stringify(body));
    this.serviceApi.post('/v1/recruitment/generate/offer-letter/', body).subscribe(
      res => {
  ...
  sendMailOffterLetter(element: any) {
    ...
    this.action = '';
    const dialogRef = this.dialog.open(SendMailOfferLetterDialogComponent, {
      width: '500px',
...
export class SendMailOfferLetterDialogComponent implements OnInit {
  ...
  generateLetterForm: FormGroup;
  constructor(public dialog: MatDialog, public dialogRef: MatDialogRef<SendMailOfferLetterDialogComponent>,
