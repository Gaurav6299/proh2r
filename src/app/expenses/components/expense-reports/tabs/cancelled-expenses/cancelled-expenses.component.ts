...
@Component({
  selector: 'app-cancelled-expenses',
  templateUrl: './cancelled-expenses.component.html',
  styleUrls: ['./cancelled-expenses.component.scss']
})
export class CancelledExpensesComponent implements OnInit {
  ...
  ngOnInit() {
    this.service.getExpenseCancelledRequest().subscribe((data) => {
      if (data != null) {
  ...
  showHistoryDialog(expDetails: any) {
    let dialogRef = this.dialog.open(ShowExpenseCancelHistoryDialogComponent, {
      width: '700px',
  ...
  showExpenseReport(expId: any) {
    this.id = expId;
    this.cancelledRequest.forEach(element => {
...
export class ShowExpenseCancelHistoryDialogComponent {
  expensesList = [];
  expenseReport = [];
  @ViewChild("dt1") dt: DataTable;
  dataColumns = [
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ShowExpenseCancelHistoryDialogComponent>,
  ...
  showHistoryDialog(element: any) {
    let dialogRef = this.innerDialog.open(ShowExpenseCancelDialogComponent, {
      width: '700px',
      panelClass: 'custom-dialog-container',
      data: element,
    });
...
export class ShowExpenseCancelDialogComponent {
  constructor(public innerDialog: MatDialog,
    public dialogRef: MatDialogRef<ShowExpenseCancelDialogComponent>,
