...
@Component({
  selector: 'app-approved-expenses',
  templateUrl: './approved-expenses.component.html',
  styleUrls: ['./approved-expenses.component.scss']
})

export class ApprovedExpensesComponent implements OnInit {
  ...
  ngOnInit() {
    this.service.getExpenseApprovedRequest().subscribe((data) => {
      if (data != null) {
  ...
  showHistoryDialog(expDetails: any) {
    let dialogRef = this.dialog.open(ShowExpenseApprovedHistoryDialogComponent, {
      width: '700px',
  ...
  showExpenseReport(expId: any) {
    this.id = expId;
    this.approvedRequest.forEach(element => {
  ...
  cancelExpApplication(expId: any) {
      ...
      const dialogRef = this.dialog.open(CancelExpenseRequest, {
        width: '500px',
        panelClass: 'custom-dialog-container',
...
export class CancelExpenseRequest implements OnInit {
  action: any;
  error: any;
  constructor(public dialogRef: MatDialogRef<CancelExpenseRequest>, @Inject(MAT_DIALOG_DATA) public data: any, private serviceApi: ApiCommonService) { }
  cancelRequest() {
...
export class ShowExpenseApprovedHistoryDialogComponent {
  expensesList = [];
  expenseReport = [];
  @ViewChild("dt1") dt: DataTable;
  dataColumns = [
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ShowExpenseApprovedHistoryDialogComponent>,
  ...
  showHistoryDialog(element: any) {
    let dialogRef = this.innerDialog.open(ShowExpenseApprovedDialogComponent, {
      width: '700px',
      panelClass: 'custom-dialog-container',
      data: element,
    });
...
export class ShowExpenseApprovedDialogComponent {
  constructor(public innerDialog: MatDialog,
    public dialogRef: MatDialogRef<ShowExpenseApprovedDialogComponent>,
