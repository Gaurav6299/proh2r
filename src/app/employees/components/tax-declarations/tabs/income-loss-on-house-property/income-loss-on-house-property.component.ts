...
@Component({
  ...
  styleUrls: ['./income-loss-on-house-property.component.scss']
})
export class IncomeLossOnHousePropertyComponent implements OnInit {
  ...
  getInterestOnHomeLoan() {
    this.interestOnHomeLoan = [];
    this.interestOnHomeLoanCopy = [];
    this.serviceApi.get("/v1/employee/taxDeclaration/getData/" + this.employeeTaxDeclarationId + "/2").subscribe(res => {
      res.categoryComponentList.forEach(element => {
        this.interestOnHomeLoan.push({
      ...
      this.interestOnHomeLoan.forEach(val => this.interestOnHomeLoanCopy.push(Object.assign({}, val)));
    }, (err) => {
  ...
  viewTrasactionHistoryDialog(event: any) {
    let dialogRef = this.dialog.open(ViewIncomeLossHousePropertyTransactionHistoryDialogComponent, {
      width: '700px',
  ...
  uploadAttachmentDialog(event: any) {
    let dialogRef = this.dialog.open(UploadIncomeLossHousePropertyFileAttachmentDialogComponent, {
      width: '600px',
      panelClass: 'custom-dialog-container',
      data: event
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result !== undefined) {
        if (result.message) {
          if (result.status === 'Response') {
            this.successNotification(result.message);
            this.interestOnHomeLoan.forEach((element, index) => {
              if (element.empTaxDeclareCompDetailId == event.empTaxDeclareCompDetailId) {
                this.interestOnHomeLoan[index].attachmentLink = result.docId;
                this.interestOnHomeLoan[index].attachmentName = result.fileName;
              }
            });
          }
  ...
  saveComponents(rowData: any) {
    ...
    this.serviceApi.put("/v1/employee/taxDeclaration/update", body).subscribe(res => {
      ...
      // this.getInterestOnHomeLoan();
      this.successNotification("Interest On Home Loan updated successfully.");
      this.interestOnHomeLoan.forEach(element => {
  ...
  resetRow(event: any) {
    var temp = this.interestOnHomeLoanCopy.filter(element2 => event.empTaxDeclareCompDetailId == element2.empTaxDeclareCompDetailId)[0];
    this.interestOnHomeLoan.forEach(element1 => {
...
export class ViewIncomeLossHousePropertyTransactionHistoryDialogComponent implements OnInit {
  action: any;
  error: any;
  columns: any[] = [
  ...
  constructor(public dialogRef: MatDialogRef<ViewIncomeLossHousePropertyTransactionHistoryDialogComponent>, @Inject(MAT_DIALOG_DATA) public data: any, private serviceApi: ApiCommonService) {
...
export class UploadIncomeLossHousePropertyFileAttachmentDialogComponent implements OnInit {
  action: any;
  error: any;
  ...
  fileName: any;
  empCode: any;
  constructor(private uploadService: UploadFileService, public dialogRef: MatDialogRef<UploadIncomeLossHousePropertyFileAttachmentDialogComponent>, @Inject(MAT_DIALOG_DATA) public data: any, private route: ActivatedRoute) {
    this.route.queryParams.subscribe(res =>
      this.empCode = res.empCode);
  }
