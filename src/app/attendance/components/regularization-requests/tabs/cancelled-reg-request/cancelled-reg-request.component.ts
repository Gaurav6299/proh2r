...
@Component({
  selector: 'app-cancelled-reg-request',
  templateUrl: './cancelled-reg-request.component.html',
  styleUrls: ['./cancelled-reg-request.component.scss']
})
export class CancelledRegRequestComponent implements OnInit {
  ...
  ngOnInit() {
    let tempStatus = '';
    this.service.getRegularizationCancelledRequest().subscribe((data) => {
      if (data != null) {
  ...
  viewRegularizationRequest(regDetails: any) {
    let dialogRef = this.dialog.open(ViewRegularizationCancelledReqComponent, {
      width: '500px',
...
export class ViewRegularizationCancelledReqComponent {
  constructor(public dialogRef: MatDialogRef<ViewRegularizationCancelledReqComponent>,
